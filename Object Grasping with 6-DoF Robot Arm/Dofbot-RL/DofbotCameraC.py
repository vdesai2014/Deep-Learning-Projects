...
box = p.loadURDF(box_path, basePosition=[0,0.13,0.01], baseOrientation = baseOrn, useFixedBase=True) #0.075 to 0.125 for Y
for i in range(100):
        p.stepSimulation()
...
rotX = p.addUserDebugParameter("Camera Rotation X", -100, 100, 0)
rotY = p.addUserDebugParameter("Camera Rotation Y", -100, 100, 0)
rotZ = p.addUserDebugParameter("Camera Rotation Z", -100, 100, 0)
transX = p.addUserDebugParameter("Camera Translation X", -3.5, 3.5, 0)
transY = p.addUserDebugParameter("Camera Translation Y", -3.5, 3.5, 0)
transZ = p.addUserDebugParameter("Camera Translation Z", -3.5, 3.5, 0)
...
while(True):
    ...
    RotX = p.readUserDebugParameter(rotX)*0.1#0.5
    RotY = p.readUserDebugParameter(rotY)*0.1
    RotZ = p.readUserDebugParameter(rotZ)*0.1
    TransX = p.readUserDebugParameter(transX)*0.1
    TransY = p.readUserDebugParameter(transY)*0.1#0.065##-0.04248653+0.07568253#
    TransZ = p.readUserDebugParameter(transZ)*0.1#0.04345
...
# Extract the camera's final position in world frame:
    cam_pos = np.dot(h_world_robot, np.append(_h_robot_camera[:3, 3], 1))[:3]

# Extract the camera's final orientation matrix in world frame:
    cam_orient_matrix = np.dot(h_world_robot[:3, :3], _h_robot_camera[:3, :3])

# Apply this orientation matrix to the z-axis unit vector:
    cam_direction = np.dot(cam_orient_matrix, [0, 0, 1])

# Define the length of the debug line (laser beam):
    beam_length = 1.0  # or whatever length you want

# Compute the end point of the debug line in world frame:
    line_end = cam_pos + beam_length * cam_direction

# Create the debug line:
    p.addUserDebugLine(lineFromXYZ=cam_pos.tolist(), lineToXYZ=line_end.tolist(), lineColorRGB=[1, 0, 0], lifeTime=1)
