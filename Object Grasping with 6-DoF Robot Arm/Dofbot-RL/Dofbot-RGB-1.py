...
class DofbotRL(gym.Env):
    ...
    def __init__(self, renders=False):
        ...
        self.observation_space = spaces.Box(low=0,
                                        high=1,
                                        shape=(3, self.height, self.width), dtype=np.float32)
        if self._renders:
    ...
    def getObservation(self):
        ...
        projectionMatrix = self.physicsClient.computeProjectionMatrixFOV(
            ...
            nearVal=0.02,
            farVal=3.0)
        _, _, rgbPixels, _, _ = self.physicsClient.getCameraImage(
            width=64, 
            height=64,
            viewMatrix=gl_view_matrix,
            projectionMatrix=projectionMatrix)
        rgb_array = np.array(rgbPixels, dtype=np.float32)[:,:,:3]
        rgb_array_normalized = rgb_array / 255.0
        rgb_array_normalized =  rgb_array_normalized.transpose(2, 1, 0)

        return rgb_array_normalized
    ...
    def endOfEpisode(self):
        ...
        for i in range(5):
            print("This episode was a " + str(reward) + " and the difficulty level was " + str(self.difficultyLevel) + " and the box lower was 0.125.")
        if(reward == 1):	
